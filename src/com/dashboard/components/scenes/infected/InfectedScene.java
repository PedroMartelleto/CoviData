...
public class InfectedScene extends AnchorPane {
	...
	private void onStateNameChange() {
...
//		TimeSeriesDataProvider.deaths(casesChart, 5, state);
	}
	...
	public void initMap() {
		...
		casesMapView.initializedProperty().addListener((observable, oldValue, newValue) -> {
			if (newValue) addCirclesToMap(casesMapView, casesMapCircles);
		});
		
		deathsMapView.initializedProperty().addListener((observable, oldValue, newValue) -> {
			if (newValue) addCirclesToMap(deathsMapView, deathsMapCircles);
		});
		
		// Initializes the map
		casesMapView.initialize(Configuration.builder().projection(Projection.WEB_MERCATOR).interactive(false).build());
		deathsMapView.initialize(Configuration.builder().projection(Projection.WEB_MERCATOR).interactive(false).build());
	}
